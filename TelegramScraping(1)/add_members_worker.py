...
class AddMembersWorker():
    def __init__(self, api_keys, group_url,root, update_status_callback, 
                 ...
                 update_progress_bar_callback, clients,clients1, delay, 
    ...
    def get_clients_from_sessions(self,client_phones:list):
        clients = []
        ...
        for session in self.sessions:
            try:
                ...
                if not self.phone.split("\\")[-1] in client_phones:
                    continue
    ...
    async def get_admins(self,client:TelegramClient,group):
        if self.groups != "":
    ...
    async def scrape_users_to_group(self,client:TelegramClient,start_offset,end_offset,group):
        if self.groups != "":
            group = await client.get_entity(self.groups)
        ...
        while True:
            ...
            try:
                ...
                # self.update_status("Some session of these sessions didn't join this group")
                print(f"Error fetching message for group :{e}")
            ...
            for message in history.messages:
                try:
                    ...
                    if message.media and message.photo:
                        file_path = await client.download_media(message.photo, file=f"photos/{username}_photo_{message.id}.jpg")
                        self.update_status(f"{username} (photo) is scraped from group")
                    ...
                    with open("scrape_users.txt","w",encoding="utf-8") as file:
                        for users in self.all_users:
                            file.write(f"{users}\n")
                except Exception as e:
    ...
    async def get_message_id_range(self,client:TelegramClient,group):
        if self.groups != "":
            group = await client.get_entity(self.groups)
    ...
    def run(self):
        ...
        clients = []
        try:
            clients = self.get_clients_from_sessions(self.clients)
            # clients = self.clients
            if not self.groups and self.run_mode=="add_mode":
            ...
            if self.run_mode == "add_mode":
                ...
                self.worker = JoinGroupWorker(self.groups, self.phones, self.update_status, self.is_stop_requested, self.update_progress_bar, self.proxies, self.apis,False)
                ...
                latest_ID,oldest_ID = loop.run_until_complete(self.get_message_id_range(clients[0],group))
                clients_len = len(clients)
                temp = (oldest_ID-latest_ID)//clients_len
                self.admin_ids = loop.run_until_complete(self.get_admins(clients[0],group))
                for i, client in enumerate(clients):
                    start_offset = i*temp+latest_ID
                    end_offset = (i+1)*temp+latest_ID
                    if end_offset>oldest_ID:
                        end_offset=oldest_ID
                    # Assign each client a different range of message IDs
                    tasks.append(self.scrape_users_to_group(client, start_offset, end_offset,group))

                # Run all clients in parallel
                results = loop.run_until_complete(asyncio.gather(*tasks))

                # Combine the results (usernames) from all clients
                all_usernames = set()
                for result in results:
                    all_usernames.update(result)

                # Print all collected usernames
                with open('scrape_users.txt', 'w', encoding='utf-8') as f:
